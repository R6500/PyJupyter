...
History:
  ...
  15/04/2018 : Add hook to plot functions
   2/07/2018 : Add interactive plots (Don't work in colaboratory)
               Add aspect parameter to plot11, plot1n and plotnn
               Add colors parameter to plotnn
'''
...
Used by the plot11, plot1n and plotnn commands
'''
def plotXY(x,y,label="",logx=False,logy=False,color=None):
    if not logx and not logy:
        pl.plot(x,y,label=label,color=color)
        return
    if logx and not logy:
        pl.semilogx(x,y,label=label)
        return
...
Optional parameters:
  title : Plot title (Defaults to none)
   ...
   aspect : If True, keep same x and y aspect ratio   

Returns nothing
'''
def plot11(x,y,title="",xt="",yt="",logx=False,logy=False,grid=True,hook=None,xlim=None,ylim=None,aspect=False):
    ...
    if aspect:
        plt.axis('equal',facecolor="white")    
    if xlim != None:
        plt.xlim(xlim[0],xlim[1])
    if ylim != None:
        plt.ylim(ylim[0],ylim[1])    
...
def plot1n(x,ylist,title="",xt="",yt="",labels=[],location='best'
            ,logx=False,logy=False,grid=True,hook=None,xlim=None,ylim=None,aspect=False):
    # Check for x, y given as strings
    if type(x)==str:
        if xt=='': xt=x
        x = getVar(x,level=2)
...
def plotnn(xlist,ylist,title="",xt="",yt="",labels=[],location='best'
           ,logx=False,logy=False,grid=True,hook=None,xlim=None,ylim=None,aspect=False,colors=[]):
    # Check for x, y given as strings
    if type(xlist[0])==str:
        if xt=='': xt=xlist[0]
    if type(ylist[0])==str and (labels==[]): 
    ...
    for i in range(0,len(xlist)):
        ...
        if labels == [] and colors == []:
            ...
            plotXY(x,y,logx=logx,logy=logy,color=colors[i])
        elif colors == []:
            plotXY(x,y,label=labels[i],logx=logx,logy=logy)
        else:
            plotXY(x,y,label=labels[i],logx=logx,logy=logy,color=colors[i])
    '''    
    if labels == []:
        for x,y in zip(xlist,ylist):
            plotXY(x,y,logx=logx,logy=logy)
